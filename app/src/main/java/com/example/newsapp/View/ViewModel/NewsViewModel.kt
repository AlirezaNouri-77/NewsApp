...
import com.example.newsapp.View.util.convertToStringForUrl
...
class NewsViewModel(
  ...
  val newsMutableList: SnapshotStateList<Article> = mutableStateListOf()
  var canPaging = true
	  ...
	  when (state) {
		...
		is BaseViewModelContract.BaseEvent.GetData -> {
		  newsRepository.getNews(
			...
			settingCategory = activeSection.value.name.lowercase(
			  Locale.getDefault()
			),
			settingQuery = settingList.convertToStringForUrl(),
		  ).collectLatest {
			_baseState.value = it
		  }
		}
		...
		is BaseViewModelContract.BaseEvent.GetSettings -> {
		  val result =
			async(context = Dispatchers.IO) { databaseRepository.getSettings() }.await()
		  
		  if (result.isNotEmpty()) {
			activeSection.value = result.getOrNull(0)?.activeSettingSection
			  ?: ActiveSettingSection.LANGUAGE
			settingList =
			  result.getOrNull(0)?.settingList?.toMutableStateList()
				?: mutableListOf(
				  SettingModel(
					name = "English",
					code = "en"
